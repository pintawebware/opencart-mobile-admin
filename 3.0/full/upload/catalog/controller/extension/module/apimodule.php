...
class ControllerExtensionModuleApimodule extends Controller
{
    ...
    public function orders()
    {
        ...
        if (isset($_REQUEST['filter'])) {
            ...
            $orders = $this->model_extension_module_apimodule->getOrders(array('filter' => $_REQUEST['filter'], 'page' => $page, 'limit' => $limit));
            ...
            $orders = $this->model_extension_module_apimodule->getOrders(array('filter' => $filter, 'page' => $page, 'limit' => $limit));

        } else {
            $orders = $this->model_extension_module_apimodule->getOrders(array('page' => $page, 'limit' => $limit));
        }
        $response = [];
        $orders_to_response = [];
        ...
        if (isset($_REQUEST['order_id']) && $_REQUEST['order_id'] != '') {
            ...
            if (count($products) > 0) {
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    $discount_price = $this->model_extension_module_apimodule->getProductDiscount($products[$i]['product_id'], $products[$i]['quantity']);
    ...
    public function login()
    {
        ...
        if (isset($this->request->post['device_token']) && $this->request->post['device_token'] != '' &&
            ...
            if ($matches == 0) {
                $this->model_extension_module_apimodule->setUserDeviceToken($user['user_id'], $_REQUEST['device_token'], $_REQUEST['os_type']);
            }
        }
    ...
    public function statistic()
    {
        ...
        if (isset($_REQUEST['filter']) && $_REQUEST['filter'] != '') {
            ...
            $orders = $this->model_extension_module_apimodule->getTotalOrders(array('filter' => $_REQUEST['filter']));
           ...
           // $data['total_sales'] = number_format($sale_total, 2, '.', '');
            ...
            $sale_year_total = $this->model_extension_module_apimodule->getTotalSales(array('this_year' => true));
