...
$this->load->model('setting/setting');
	...
	$this->model_setting_setting->editSetting('module_apimodule', ['module_apimodule_status'=>1,'version'=>1.7]);
if(VERSION == '2.0.0.0'){
    $this->load->model('tool/event');
	$this->model_tool_event->deleteEvent('apimodule');
	$this->model_tool_event->addEvent('apimodule', 'post.order.history.add', 'extension/module/apimodule/sendNotifications');
}else{
	$this->load->model('extension/event');
	$this->model_extension_event->deleteEvent('apimodule');
	$this->model_extension_event->addEvent('apimodule', 'catalog/model/checkout/order/addOrderHistory/after', 'extension/module/apimodule/sendNotifications');
}
